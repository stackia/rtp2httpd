...
function VolumeIndicator({ volume, label }: { volume: number; label: string }) {
  const percent = Math.round(volume * 100);
  return (
    <>
      {volume <= 0 ? (
        <VolumeX className={ICON_CLASS} aria-label={label} />
      ) : volume < 0.5 ? (
        <Volume1 className={ICON_CLASS} aria-label={label} />
      ) : (
        <Volume2 className={ICON_CLASS} aria-label={label} />
      )}
...
function ChannelIndicator({
  ...
  return (
    <>
      <Chevron className={ICON_CLASS} aria-label={label} />
      {target ? (
        <>
...
export function PlayerGestureIndicatorOverlay({
  ...
  // Keep the last indicator on screen while the card fades out, so the content does not
  // blink away before the opacity transition finishes.
  const lastIndicatorRef = useRef<PlayerGestureIndicator | null>(null);
  if (indicator) lastIndicatorRef.current = indicator;
  const shown = indicator ?? lastIndicatorRef.current;
    ...
    <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="relative z-10 flex min-w-0 items-center gap-2 md:gap-3">
          {shown?.kind === "volume" && <VolumeIndicator volume={shown.volume} label={t("volume")} />}
          {shown?.kind === "channel" && (
            <ChannelIndicator
              indicator={shown}
          ...
          {shown?.kind === "seek" && (
            <>
              {shown.deltaSeconds < 0 ? (
                <Rewind className={ICON_CLASS} aria-hidden />
              ) : (
                <FastForward className={ICON_CLASS} aria-hidden />
              )}
