...
int fcc_telecom_initialize_and_request(stream_context_t *ctx)
{
    ...
    r = sendto_triple(fcc->fcc_sock,
                      build_fcc_request_pk_telecom(ctx->service->addr, fcc->fcc_client.sin_port),
                      FCC_PK_LEN_REQ_TELECOM, 0,
                                       ...
                                       struct sockaddr_in *peer_addr)
{
    ...
    logger(LOG_DEBUG, "FCC Response: FMT=3, result=%u, signal_port=%u, media_port=%u, valid_time=%u, speed=%s, speed_after_sync=%s",
           result_code, ntohs(new_signal_port), ntohs(new_media_port), valid_time, speed_str, speed_after_sync_str);
    ...
    {
        logger(LOG_DEBUG, "FCC (Telecom): Server provided new signal port: %u", ntohs(new_signal_port));
    ...
    if (new_media_port && new_media_port != fcc->media_port)
    {
        ...
        logger(LOG_DEBUG, "FCC (Telecom): Server provided new media port: %u", ntohs(new_media_port));
    ...
    else if (type == 3)
    {
        ...
        logger(LOG_DEBUG, "FCC (Telecom): Server requests redirection to new server %s:%u (redirect #%d)",
...
int fcc_telecom_send_term_packet(fcc_session_t *fcc, service_t *service, uint16_t seqn, const char *reason)
{
    ...
    r = sendto_triple(fcc->fcc_sock,
                      build_fcc_term_pk_telecom(service->addr, seqn),
                      FCC_PK_LEN_TERM_TELECOM, 0,
                      fcc->fcc_server, sizeof(*fcc->fcc_server));
    if (r < 0)
    {
