...
int fcc_telecom_initialize_and_request(stream_context_t *ctx) {
  ...
  r = sendto_triple(fcc->fcc_sock, build_fcc_request_pk_telecom(ctx->service->addr, fcc->fcc_client.sin_port),
                    FCC_PK_LEN_REQ_TELECOM, 0, fcc->fcc_server, sizeof(*fcc->fcc_server));
  if (r < 0) {
    logger(LOG_ERROR, "FCC (Telecom): Unable to send request message: %s", strerror(errno));
    return -1;
  }
  logger(LOG_DEBUG, "FCC (Telecom): Request (FMT 2) sent to server %s:%u", inet_ntoa(fcc->fcc_server->sin_addr),
...
int fcc_telecom_handle_server_response(stream_context_t *ctx, uint8_t *buf, size_t buf_len) {
  ...
  if (fmt == FCC_FMT_TELECOM_SYN) {
    ...
    if (buf_len < 36) {
      logger(LOG_WARN, "FCC Telecom: FMT 3 response too short (%zu bytes, need 36)", buf_len);
      return 0;
    }
  ...
  if (speed_after_sync >= 1048576) {
    snprintf(speed_after_sync_str, sizeof(speed_after_sync_str), "%.2f Mbps", speed_after_sync / 1048576.0);
  } else if (speed_after_sync >= 1024) {
    snprintf(speed_after_sync_str, sizeof(speed_after_sync_str), "%.2f Kbps", speed_after_sync / 1024.0);
  } else {
    snprintf(speed_after_sync_str, sizeof(speed_after_sync_str), "%u bps", speed_after_sync);
  }
  logger(LOG_DEBUG,
         "FCC Response: FMT=3, result=%u, signal_port=%u, media_port=%u, "
         "valid_time=%u, speed=%s, speed_after_sync=%s",
         result_code, ntohs(new_signal_port_be), ntohs(new_media_port_be), valid_time, speed_str, speed_after_sync_str);
  ...
  if (new_signal_port_be && new_signal_port_be != fcc->fcc_server->sin_port) {
    ...
    logger(LOG_DEBUG, "FCC (Telecom): Server provided new signal port: %u", ntohs(fcc->fcc_server->sin_port));
    signal_port_changed = 1;
  }

  if (new_media_port_be && new_media_port_be != fcc->media_port) {
    fcc->media_port = new_media_port_be;
    logger(LOG_DEBUG, "FCC (Telecom): Server provided new media port: %u", ntohs(fcc->media_port));
  ...
  if (new_fcc_ip_be != 0) {
    ...
    if (new_fcc_ip_be != fcc->fcc_server->sin_addr.s_addr) {
      ...
      logger(LOG_DEBUG, "FCC (Telecom): Server provided new IP: %s", inet_ntoa(fcc->fcc_server->sin_addr));
      signal_port_changed = 1;
      media_port_changed = 1;
    }
  }
  ...
  if (type == 1) {
    ...
    logger(LOG_DEBUG, "FCC (Telecom): Server accepted request, waiting for unicast stream");
  } else if (type == 3) {
    /* Redirect to new FCC server */
    fcc->redirect_count++;
    if (fcc->redirect_count > FCC_MAX_REDIRECTS) {
      logger(LOG_WARN, "FCC (Telecom): Too many redirects (%d), falling back to multicast", fcc->redirect_count);
    ...
    logger(LOG_WARN, "FCC (Telecom): Unsupported type=%u, falling back to multicast", type);
...
int fcc_telecom_send_term_packet(fcc_session_t *fcc, service_t *service, uint16_t seqn, const char *reason) {
  ...
  if (!service) {
    logger(LOG_DEBUG, "FCC (Telecom): Cannot send termination - missing service");
    return -1;
  }

  /* Telecom FCC: Send standard termination packet (FMT 5) */
  r = sendto_triple(fcc->fcc_sock, build_fcc_term_pk_telecom(service->addr, seqn), FCC_PK_LEN_TERM_TELECOM, 0,
                    fcc->fcc_server, sizeof(*fcc->fcc_server));
  if (r < 0) {
    logger(LOG_ERROR, "FCC (Telecom): Unable to send termination packet (FMT 5) (%s): %s", reason, strerror(errno));
    return -1;
  }
  logger(LOG_DEBUG, "FCC (Telecom): Termination packet (FMT 5) sent (%s), seqn=%u", reason, seqn);
