...
static int deliver_raw_packet(uint8_t *data, int len, connection_t *conn, int is_snapshot) {
  if (is_snapshot) {
...
static int flush_consecutive(rtp_reorder_t *r, connection_t *conn, int is_snapshot, int log_recovery,
                             fec_context_t *fec) {
  int total_bytes = 0;
  int flushed = 0;
  ...
  if (log_recovery && flushed > 1) {
    logger(LOG_DEBUG, "RTP reorder: Recovered %d out-of-order packets (seq %u-%u)", flushed, start_seq,
           (uint16_t)(r->base_seq - 1));
  }
  ...
  if (fec && fec->min_end_seq_valid && (int16_t)(r->base_seq - fec->min_end_seq) > 0) {
...
static int force_flush_until(rtp_reorder_t *r, uint16_t target_seq, connection_t *conn, int is_snapshot,
                             fec_context_t *fec) {
  int total_bytes = 0;
  int lost_count = 0;
  uint16_t start_seq = r->base_seq;
  ...
  if (lost_count > 0) {
    logger(LOG_DEBUG, "RTP reorder: Packet loss at seq %u (target=%u)", start_seq, target_seq);
...
int rtp_reorder_insert(rtp_reorder_t *r, buffer_ref_t *buf_ref, uint16_t seqn, connection_t *conn, int is_snapshot,
                       fec_context_t *fec) {
  int total_bytes = 0;
  ...
  if (unlikely(r->phase == 1)) {
    ...
    if (r->count >= RTP_REORDER_INIT_COLLECT) {
      ...
      logger(LOG_DEBUG, "RTP reorder: Init complete, base_seq=%u (%d packets collected)", r->base_seq, r->count);
  ...
  if (fec && fec_is_enabled(fec)) {
    ...
    if (fec_attempt_recovery(fec, r->base_seq, &recovered_data, &recovered_len) == 0) {
      /* Recovery succeeded! Deliver the recovered packet */
      int bytes = deliver_raw_packet(recovered_data, recovered_len, conn, is_snapshot);
      if (bytes > 0)
        total_bytes += bytes;
      free(recovered_data);
...
void rtp_reorder_release_range(rtp_reorder_t *r, uint16_t begin_seq, uint16_t end_seq) {
