...
int service_url_has_template(const char *url) {
  ...
  /* Check for ${...} patterns (covers ${(b)...}, ${(e)...}) */
  if (strstr(url, "${"))
    return 1;

  /* Check for known simple placeholders */
  static const char *tokens[] = {
    "{utc}", "{start}", "{lutc}", "{end}", "{duration}", "{offset}",
    "{utcend}", "{now}", "{timestamp}",
    "{Y}", "{m}", "{d}", "{H}", "{M}", "{S}",
    NULL
  };
  ...
  const char *pb = url;
  while ((pb = strstr(pb, "{(")) != NULL) {
    if (pb == url || *(pb - 1) != '$') {
      if ((pb[2] == 'b' || pb[2] == 'e') && pb[3] == ')')
        return 1;
    }
    pb++;
  }

  /* Check for {keyword:FORMAT} patterns (without $ prefix) */
...
static int format_iso8601_utc(const struct tm *tm, char *output,
                              size_t output_size) {
  int written;

  if (!tm || !output || output_size == 0)
    return -1;

  written = snprintf(output, output_size, "%04d-%02d-%02dT%02d:%02d:%02d.000Z",
...
/* Time component lookup table for ${yyyy}/${MM}/... and {Y}/{m}/... */
typedef struct {
  const char *long_name;  /* ${...} syntax */
  const char *short_name; /* {...} syntax */
  const char *fmt;
} time_component_def_t;

static const time_component_def_t time_components[] = {
  {"yyyy", "Y", "%04d"}, /* tm_year + 1900 */
  {"MM",   "m", "%02d"}, /* tm_mon + 1 */
  {"dd",   "d", "%02d"}, /* tm_mday */
  {"HH",   "H", "%02d"}, /* tm_hour */
  {"mm",   "M", "%02d"}, /* tm_min */
...
static int get_tm_component(const struct tm *t, int index) {
  switch (index) {
  case 0: return t->tm_year + 1900;
  case 1: return t->tm_mon + 1;
...
/*
 * Render a (b)/(e) format placeholder.
 * Handles: bare → ISO8601, "timestamp" → epoch, FORMAT[|UTC] → formatted.
 */
static int render_be_format(const char *fmt, size_t fmt_len, int is_begin,
  ...
  if (fmt_len == 0) {
    return format_iso8601_utc(tm_utc, val, val_size);
  }
...
/*
 * Render a keyword placeholder (keyword, keyword:FORMAT, or component).
 * Returns: 1 = handled, 0 = not a known keyword, -1 = error.
 */
static int render_keyword(const char *inner, int use_short,
  ...
  /* Bare keyword → ISO8601 or epoch */
  if (strcmp(inner, "utc") == 0 || strcmp(inner, "start") == 0) {
    if (!ctx->has_begin ||
        format_iso8601_utc(&ctx->begin_tm_utc, val, val_size) != 0)
      return -1;
    return 1;
  }
  if (strcmp(inner, "utcend") == 0 || strcmp(inner, "end") == 0) {
    if (!ctx->has_end ||
        format_iso8601_utc(&ctx->end_tm_utc, val, val_size) != 0)
      return -1;
    return 1;
  }
  if (strcmp(inner, "lutc") == 0 || strcmp(inner, "now") == 0) {
    if (format_iso8601_utc(now_tm, val, val_size) != 0)
      return -1;
    return 1;
  }
  ...
  /* Time component lookup */
  for (int i = 0; time_components[i].long_name; i++) {
    const char *name =
        use_short ? time_components[i].short_name : time_components[i].long_name;
  ...
  return 0; /* Not a known keyword */
}

/*
 * Try to match and render a placeholder at position p.
 * inner_offset: 2 for ${...}, 1 for {...}
 * be_char_offset: 3 for ${(b)...}, 2 for {(b)...}
 * fmt_offset: 5 for ${(b)FMT}, 4 for {(b)FMT}
 * use_short: 0 for ${} long format, 1 for {} short format
 *
 * Returns: >0 = chars consumed, 0 = not matched, -1 = error.
 */
static int render_placeholder(const char *p, int inner_offset,
  ...
  /* Try keyword:FORMAT / keyword / component */
  {
    ...
    if (closing) {
      ...
      int rc = render_keyword(inner, use_short, ctx, now_tm, val,
                              sizeof(val));
      if (rc < 0)
        return -1;
      if (rc > 0) {
...
static int render_template_url(const char *url, const seek_context_t *ctx,
  ...
  while (*p) {
    ...
    /* {...} patterns without $ prefix (short format) */
    if (*p == '{' && (p == url || *(p - 1) != '$')) {
...
static int format_time_by_pattern(const struct tm *t, const char *fmt,
  ...
  while (*p && pos < output_size - 1) {
    if (use_short) {
      /* Short format: Y→year, m→month, d→day, H→hour, M→minute, S→second */
      if (*p == 'Y') {
        int n = snprintf(output + pos, output_size - pos, "%04d",
                         t->tm_year + 1900);
        ...
        int n = snprintf(output + pos, output_size - pos, "%02d",
                         t->tm_mon + 1);
        if (n < 0 || (size_t)n >= output_size - pos)
          return -1;
        pos += n;
        ...
        int n = snprintf(output + pos, output_size - pos, "%02d", t->tm_mday);
        if (n < 0 || (size_t)n >= output_size - pos)
          return -1;
        pos += n;
        p += 1;
      } else if (*p == 'H') {
        int n = snprintf(output + pos, output_size - pos, "%02d", t->tm_hour);
        if (n < 0 || (size_t)n >= output_size - pos)
          return -1;
        pos += n;
        p += 1;
      } else if (*p == 'M') {
        int n = snprintf(output + pos, output_size - pos, "%02d", t->tm_min);
        if (n < 0 || (size_t)n >= output_size - pos)
          return -1;
        pos += n;
        p += 1;
      } else if (*p == 'S') {
        int n = snprintf(output + pos, output_size - pos, "%02d", t->tm_sec);
        if (n < 0 || (size_t)n >= output_size - pos)
          return -1;
        pos += n;
        p += 1;
      ...
      /* Long format: yyyy→year, MM→month, dd→day, HH→hour, mm→min, ss→sec */
...
int service_resolve_upstream_url(const char *url,
  ...
  has_template = service_url_has_template(url);

  if (seek_context_init(&ctx, seek_param_value, tz_offset,
  ...
  if (!has_template) {
    ...
    if (seek_param_name && ctx.has_seek &&
  ...
  if (!ctx.has_seek) {
    /* No seek value: copy URL as-is (placeholders left for player) */
    strncpy(output, url, output_size - 1);
    output[output_size - 1] = '\0';
    return 0;
  }
