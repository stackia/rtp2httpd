...
static const uint8_t fec_g0_k100[100] = {
    0x20, 0x85, 0x89, 0xdf, 0xd0, 0x65, 0xb2, 0xf4, 0x44, 0x56, 0x3f, 0x25, 0x73, 0x51, 0x48, 0x62, 0x75,
    0x74, 0x91, 0x7d, 0xfa, 0xaf, 0x25, 0x9d, 0xdf, 0x9b, 0x94, 0xc7, 0x53, 0x65, 0xa0, 0x81, 0xde, 0x07,
    0xf8, 0x3f, 0x09, 0x94, 0xdf, 0x5c, 0x7d, 0x3e, 0x24, 0xc5, 0xd4, 0x7c, 0xf4, 0xfe, 0x71, 0x7f, 0x2d,
    0x46, 0x43, 0x0e, 0x10, 0x38, 0xe6, 0xeb, 0x45, 0x90, 0x25, 0xc2, 0x08, 0x37, 0xe0, 0x9e, 0xb7, 0x84,
    0x55, 0x98, 0x50, 0xd0, 0xf1, 0x15, 0x30, 0xdf, 0xa2, 0x7c, 0x3b, 0xee, 0x6b, 0xaa, 0xc8, 0x20, 0xce,
    0xf9, 0x38, 0x26, 0xb7, 0x80, 0x76, 0x2f, 0xa5, 0xeb, 0x33, 0x13, 0x1b, 0x1c, 0x12, 0x14};

static const uint8_t fec_g4_k100[100] = {
    0x3c, 0x66, 0xa8, 0x11, 0xcc, 0xfa, 0x41, 0x6f, 0x0a, 0x26, 0x39, 0xf6, 0xa0, 0xab, 0x97, 0x84, 0x82,
    0x69, 0xd4, 0x43, 0xb9, 0xdc, 0x1d, 0xfe, 0xb0, 0xe2, 0x4e, 0x60, 0x08, 0x83, 0x29, 0x24, 0xdb, 0xd8,
    0xce, 0x12, 0x8d, 0x99, 0x4b, 0x3c, 0x8b, 0x14, 0x0d, 0x4d, 0xa4, 0x60, 0xe0, 0x47, 0xeb, 0xe8, 0x70,
    0x17, 0x20, 0xfd, 0x61, 0x65, 0x45, 0xc2, 0xcc, 0x2f, 0xc7, 0xce, 0xb5, 0x65, 0x0d, 0x74, 0x26, 0x32,
    0x39, 0x76, 0x3a, 0x5e, 0x21, 0x20, 0xb5, 0xd9, 0x75, 0x59, 0xf5, 0xf2, 0x3d, 0xb4, 0x91, 0x50, 0x23,
    0xaa, 0x74, 0xf2, 0xae, 0x94, 0x03, 0x71, 0xcd, 0xf0, 0x97, 0xa5, 0xf5, 0xe8, 0xc4, 0xd8};
 ...
 */
static uint8_t get_g_coefficient(int row, int col, const uint8_t *g0_table, const uint8_t *g4_table) {
  if (row < 4) {
...
static void matrix_mul_gf256(uint8_t **a, uint8_t **b, uint8_t **c, int left, int mid, int right) {
  int i, j, k;
  for (i = 0; i < left; ++i)
    for (j = 0; j < right; ++j)
      for (k = 0; k < mid; ++k) {
...
rs_fec_t *rs_fec_new(int data_pkt_num, int fec_pkt_num) {
  ...
  if (data_pkt_num == 100 && fec_pkt_num == 5) {
    ...
    for (i = 0; i < rs->m; ++i) {
      for (j = 0; j < rs->k; ++j) {
        rs->en_GM[i][j] = get_g_coefficient(i, j, fec_g0_k100, fec_g4_k100);
      }
    }
  } else {
    ...
    if (en_left == NULL || en_right == NULL || en_left_buf == NULL || en_right_buf == NULL) {
      free(en_left_buf);
      free(en_right_buf);
      free(en_left);
      free(en_right);
