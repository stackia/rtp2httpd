...
/*
 * Unified cleanup function to avoid code duplication (DRY principle)
 */
...
void start_rtp_stream(int client, struct services_s *service)
{
    ...
    logger(LOG_DEBUG, "Starting RTP stream processing for service type %d", service->service_type);

    /* Main processing loop */
    while (1) {
        /* Handle initial state - decide between FCC and direct multicast */
        if (ctx.fcc.state == FCC_STATE_INIT) {
