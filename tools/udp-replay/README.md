...
| Option                  | Description                                                       |
| ----------------------- | ----------------------------------------------------------------- |
| `-i, --interface IFACE` | Network interface for multicast, for example `eth0`               |
...
The replay tool detects the IGMP join and starts sending packets. When the curl request ends, it detects the IGMP leave and stops.
...
3. Monitor: polls `/proc/net/igmp` every 50 ms to detect multicast group membership within monitored subnets.
4. Dynamic targeting: when any address in the monitored subnets is joined, replays packets to that address.
5. Multi-target replay: supports simultaneous multicast to multiple joined addresses.
6. Replay: sends packets with original timing, adjusted by the speed multiplier.
7. Loop: after completing one replay cycle, waits 3 seconds before the next loop unless `--continuous` is used.
8. Stop: when all groups are left, pauses until the next join.
