...
"""
    ...
    def test_xff_m3u_url_uses_https_from_forwarded_ssl(self, r2h_binary):
        """With -X, X-Forwarded-Ssl should upgrade M3U URLs to https."""
        port = find_free_port()
        config = f"""\
[global]
verbosity = 4
...
"""
        ...
        try:
            ...
            assert status == 200
            text = body.decode()
            assert "https://public.example.com/" in text
            assert "http://public.example.com/" not in text
        finally:
            r2h.stop()

    def test_xff_m3u_url_uses_https_from_forwarded_header(self, r2h_binary):
        """With -X, RFC 7239 Forwarded proto=https should upgrade M3U URLs."""
        port = find_free_port()
        config = f"""\
[global]
verbosity = 4
        ...
        try:
            ...
            status, _, body = http_get(
                ...
                headers={
                    "Host": "public.example.com",
                    "X-Forwarded-For": "203.0.113.7",
                    "Forwarded": 'for=203.0.113.7;proto=https;host="public.example.com"',
                },
            )
            assert status == 200
            text = body.decode()
