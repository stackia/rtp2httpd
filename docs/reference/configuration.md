...
- `--log-format <格式>` - 访问日志格式，使用类似 nginx 的 `$变量` 占位符
...
`access_log` 留空或不配置时不会记录访问日志。

`log_format` 支持以下占位符：

- 时间：`$time_iso8601`、`$time_local`、`$msec`
- 客户端与工作进程：`$client_addr`、`$remote_addr`、`$remote_port`、`$worker_pid`
- 请求信息：`$request`、`$request_method`、`$service_url`、`$host`、`$http_user_agent`、`$http_x_forwarded_for`
- 流信息：`$service_type`、`$upstream_url`
- 字面量：`$$` 输出 `$`

`$client_addr` 与状态页面显示的客户端地址一致。当请求中的 `X-Forwarded-For` 被接受时（例如启用 `xff` 后），它会使用 `X-Forwarded-For` 中的第一个地址；这种情况下通常没有端口，因此 `$remote_port` 输出 `-`。
...
- 工作进程会在重载时重新打开访问日志文件，便于配合 logrotate
