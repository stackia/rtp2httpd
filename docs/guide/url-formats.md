...
```url
http://服务器地址:端口/rtp/组播地址:端口[?fcc=FCC服务器:端口][&fcc-type=协议类型][&fec=FEC端口][&ifname=网络接口]
```
...
- **ifname**（可选）：指定用于接收组播流的网络接口名称（如 `eth0`, `eth1` 等）。优先级高于全局配置 `upstream-interface-multicast`
...
- 通过 `ifname` 参数指定不同频道使用不同网络接口（适用于多网卡环境，如不同接口接收不同运营商的组播流）
