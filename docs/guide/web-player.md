...
| **画质增强** | 关闭 | FSR（EASU + RCAS）锐化 / 上采样到显示分辨率 |

对**逐行**片源，自动反交错只会以较低频率做检测采样，画面仍由原生 `<video>` 直接显示，不会每帧走完整 WebGL 合成。一旦检测到隔行，才会切换到 canvas 输出并启用 BWDIF（每源帧渲染两个场，GPU 负载约为普通直通的约 2 倍）。

**画质增强**会让 canvas 路径持续工作，即使片源是逐行也会每帧上传纹理并跑 FSR，在手机上更容易发热、耗电。需要锐化或放大时再手动开启即可。

> [!TIP]
> 移动端若感觉发热明显，优先关闭 **画质增强**；若频道本身是逐行，保持 **自动反交错** 开启通常也不会带来明显额外发热。
