...
> [!IMPORTANT]
> 播放器最终依赖浏览器提供的音视频解码能力，部分编码格式（如 E-AC3）可能在某些浏览器中无法播放（表现为无音频、画面黑屏）。推荐使用最新版本的 Chrome、Edge 或 Safari。

## 播放后端

播放器会根据运行平台自动选择媒体后端，界面和交互方式保持一致：

- **MSE 后端**：用于普通桌面和移动浏览器。播放器在 Web Worker 中解析 MPEG-TS，通过 Media Source Extensions（MSE）播放，并可使用 WebGL 自动反交错和画质增强。
- **Native 后端**：用于 LG webOS 浏览器。频道 URL 直接交给浏览器的原生媒体管线播放，由电视负责 TS 解析、解码、反交错和画质增强。

Native 后端仍然支持频道列表、EPG、时移回看、进度条、线路切换、遥控器操作和全屏等播放器功能。由于反交错和画质增强完全由电视管理，设置菜单在 Native 后端下不会显示这两个选项。

后端根据浏览器的 webOS 标识自动选择，无法手动切换。如果 Native 后端播放失败，播放器会按顺序重试当前线路并尝试其他线路，但不会切换到 MSE 后端。
