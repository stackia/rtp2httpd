...
播放器设置中的 **视频处理** 选项会在浏览器端用 WebGL 对已解码画面做后处理（仅对 1080p 及以下生效）：

| 选项 | 默认 | 作用 |
| ---- | ---- | ---- |
| **自动反交错** | 开启 | GPU 启发式检测隔行内容；确认隔行后启用 BWDIF 反交错 |
| **画质增强** | 开启 | FSR（EASU + RCAS）锐化 / 上采样到显示分辨率（上采样倍率有上限，避免高 DPR 手机每帧接近 4K 开销） |

**画质增强**开启时，画面走 WebGL canvas 路径：每帧上传纹理并跑 FSR。关闭画质增强后，若片源是**逐行**，自动反交错只会以较低频率做检测采样，画面改由原生 `<video>` 直接显示（detect-only），不会每帧走完整 WebGL 合成。一旦检测到隔行，才会切换到 canvas 并启用 BWDIF（每源帧渲染两个场，GPU 负载约为普通直通的约 2 倍）。

> [!TIP]
> 移动端若感觉发热明显，可先关闭 **画质增强**（仍保留自动反交错检测）；隔行频道在启用 BWDIF 时 GPU 负载会明显升高，这是反交错算法本身的成本。
