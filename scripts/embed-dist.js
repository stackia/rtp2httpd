...
function gzipDeterministic(buffer) {
  ...
  // zlib writes the gzip OS header byte based on the current platform.
  // Normalize it so the generated header is reproducible across macOS/Linux.
  if (compressed.length > GZIP_HEADER_OS_OFFSET) {
    compressed[GZIP_HEADER_OS_OFFSET] = GZIP_OS_UNIX;
  }
